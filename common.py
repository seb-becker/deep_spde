...
def neural_net(y, neurons, name, is_training,
    ...
    def batch_normalization(x):
        ...
        if is_training:
            ...
            return tf.nn.batch_normalization(x, mv_mean, mv_var,
                                             beta, gamma, 1e-6)
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse):
        ...
        for i in range(len(neurons) - 1):
            with tf.compat.v1.variable_scope('layer_%i_' % (i + 1)):
                y = layer(y, neurons[i], activation_fn)
        with tf.compat.v1.variable_scope('layer_%i_' % len(neurons)):
            return layer(y, neurons[-1], tf.identity)


def splitting_model(y, z, t, n, phi, h, net, neurons,
                    batch_size, dtype=tf.float32, activation_fn=tf.nn.tanh):
    ...
    if net == n - 1:
        v_n = tf.compat.v1.get_variable(
            'v_%i_' % (net + 1), [], dtype,
            tf.random_uniform_initializer())
        v_j = tf.ones([batch_size, 1], dtype) * v_n
    else:
        v_j = neural_net(y[:, :, 0], neurons, 'v_%i_' % (net + 1),
                         True, dtype=dtype, activation_fn=activation_fn)

    loss = (v_j - tf.stop_gradient(
        h(_y, v_i, grad_v[0], _z, t / n))) ** 2

    return tf.reduce_mean(loss), v_n


def simulate(t, n, d, sde, phi, h, z, neurons, train_steps,
             batch_size, lr_boundaries, lr_values,
             path, epsilon=1e-8, activation_fn=tf.nn.tanh):
    ...
    for i in range(n):
        ...
        y = sde(d, n - i - 1)
        loss, v_n = splitting_model(y, z, t, n, phi, h, i,
                                    neurons, batch_size, activation_fn=activation_fn)
        ...
        update_ops = tf.compat.v1.get_collection(
            tf.compat.v1.GraphKeys.UPDATE_OPS, 'v_%i_' % (i + 1))
        ...
        with tf.compat.v1.Session() as sess:
            ...
            var_list_n = tf.compat.v1.get_collection(
                tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                'v_%i_' % (i + 1))
            saver_n = tf.compat.v1.train.Saver(var_list=var_list_n)

            if i > 0:
                saver_p = tf.compat.v1.train.Saver(
                    var_list=tf.compat.v1.get_collection(
                        tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                        'v_%i_' % i))
                saver_p.restore(sess, os.path.join(
                    path, 'model_%i_' % i))

            for _ in range(train_steps):
                sess.run(train_op)

            saver_n.save(sess, os.path.join(
                path, 'model_%i_' % (i + 1)))
            try:
